...
void AP_InertialSensor_Backend::_publish_gyro(uint8_t instance, const Vector3f &gyro)
{
    ...
    // publish delta angle
    _imu._delta_angle[instance] = _delta_angle_acc[instance];
    _imu._delta_angle_valid[instance] = true;
                                                            ...
                                                            const Vector3f &gyro)
{
    ...
    // compute delta angle
    Vector3f delta_angle = (gyro + _last_raw_gyro[instance]) * 0.5f * dt;
    ...
    Vector3f delta_coning = (_delta_angle_acc[instance] +
                             _last_delta_angle[instance] * (1.0f / 6.0f));
    delta_coning %= delta_angle;
    delta_coning *= 0.5f;
    ...
    _delta_angle_acc[instance] += delta_angle + delta_coning;

    // save previous delta angle for coning correction
    _last_delta_angle[instance] = delta_angle;
    _last_raw_gyro[instance] = gyro;
}
